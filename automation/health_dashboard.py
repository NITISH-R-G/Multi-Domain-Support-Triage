...
def _parse_bandit_results(bandit_out):
    bandit_issues = 0
    bandit_high = 0
    try:
...
def _run_safety_checks(code_dir):
...
def generate_health_dashboard():
    ...
    bandit_issues, bandit_high = _parse_bandit_results(bandit_out)

    safety_issues = _run_safety_checks(code_dir)

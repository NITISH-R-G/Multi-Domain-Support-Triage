...
def _get_bandit_metrics(code_dir):
...
def _get_safety_metrics(code_dir):
...
def _get_test_metrics(code_dir):
    test_cmd = "python -m pytest tests -q"
    _, test_rc = run_command(test_cmd, cwd=code_dir)
    return "Pass" if test_rc == 0 else "Fail"


def _calculate_health_score(bandit_high, bandit_issues, safety_issues, test_status):
    health_score = 100
    if bandit_high > 0:
        health_score -= 30
...
def generate_health_dashboard():
    ...
    bandit_issues, bandit_high = _get_bandit_metrics(code_dir)
    safety_issues = _get_safety_metrics(code_dir)
    test_status = _get_test_metrics(code_dir)

...
def _extract_comment(event_data):
...
def extract_issue_details(event_data):
    action = event_data.get("action")
    if "pull_request" in event_data and action in ["opened", "edited"]:
        return _extract_pr(event_data)
    ...
    if (
        ...
        return _extract_issue(event_data)

    if "comment" in event_data and action == "created":
        return _extract_comment(event_data)
...
def main():
    ...
    issue_number, title, body, event_type = extract_issue_details(event_data)

    if not event_type:
        print("Unsupported event or action, or ignoring bot comment.")
        return

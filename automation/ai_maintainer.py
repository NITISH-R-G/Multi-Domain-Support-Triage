...
def process_labels(repo, issue_number, token, event_type, action):
    if action == "opened":
        if event_type == "Pull Request":
            add_labels(repo, issue_number, token, ["pr:needs-review"])
        elif event_type == "Issue":
            add_labels(repo, issue_number, token, ["issue:triage"])
...
def main():
    ...
    if not issue_number:
        print("Could not determine issue number or event not supported.")
        return

    process_labels(repo, issue_number, token, event_type, action)

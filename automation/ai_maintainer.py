...
def parse_pull_request_event(event_data, action):
...
def parse_issue_event(event_data, action):
    if (
        "issue" in event_data
...
def parse_comment_event(event_data, action):
...
def parse_webhook_payload(event_data, action):
    pr_data = parse_pull_request_event(event_data, action)
    if pr_data:
        return pr_data

    issue_data = parse_issue_event(event_data, action)
    if issue_data:
        return issue_data

    comment_data = parse_comment_event(event_data, action)
    if comment_data:
        return comment_data
...
def main():
    ...
    issue_number, title, body, event_type = parse_webhook_payload(event_data, action)

    if not issue_number:
        print("Unsupported event or action, or could not determine issue number.")
        return
